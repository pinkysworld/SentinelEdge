...
- **`server_av` module extracted**: the local open-source AV signature loader — preset-directory + extension constants, `local_av_signature_files`, `local_av_signature_presets_json`, and `load_local_open_source_av_signatures` — moved out of `server.rs` into `src/server_av.rs` (~100 lines). Behaviour identical (operator-gated, no auto-download).
