...
const DEFAULT_ENROLLMENT_TOKEN_TTL_SECS: u64 = 60 * 60;
...
pub struct EnrollmentToken {
    /// Raw token material. Returned only at creation time; persisted records
    /// keep this empty and validate via `token_hash`.
    #[serde(default)]
    pub token: String,
    #[serde(default)]
    ...
    pub max_uses: u32,
    /// Optional expiration timestamp (RFC 3339). New tokens default to a TTL.
    #[serde(default)]
...
impl EnrollmentToken {
    pub fn new(max_uses: u32) -> Self {
        ...
        let expires = chrono::Utc::now()
            + chrono::Duration::seconds(DEFAULT_ENROLLMENT_TOKEN_TTL_SECS as i64);
        Self {
    ...
    fn matches_presented(&self, presented: &str) -> bool {
        let expected_hash = if self.token_hash.is_empty() {
            enrollment_token_hash(&self.token)
        } else {
            self.token_hash.clone()
        };
        ct_eq(&expected_hash, &enrollment_token_hash(presented))
    }
...
impl AgentRegistry {
    ...
    pub fn enroll(&mut self, req: &EnrollRequest) -> Result<EnrollResponse, String> {
        ...
        let token_entry = self
            .tokens
            .iter_mut()
            .find(|t| t.matches_presented(&req.enrollment_token) && t.is_valid());
        match token_entry {
            Some(t) => {
                t.consume();
            }
...
mod tests {
    ...
    fn token_no_ttl_valid() {
        ...
        assert!(token.is_valid());
        assert!(token.expires_at.is_some());
        assert_ne!(token.token_hash, token.token);
    }
    ...
    fn registry_persists_token_hash_not_raw_token() {
        ...
        assert!(reloaded.enroll(&req).is_ok());
