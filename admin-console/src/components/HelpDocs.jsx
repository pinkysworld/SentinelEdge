...
import { JsonDetails, RawJsonDetails, SummaryGrid, formatDateTime, formatRelativeTime } from './operator.jsx';

const GUIDES = {
  'threat-detection': {
    title: 'Detection Support',
    intro: 'Use this support view to decide whether a rule needs tuning, suppression, validation replay, or promotion.',
    steps: [
  ...
  infrastructure: {
    ...
    intro: 'The infrastructure explorer now treats exposure, integrity, and telemetry as one operator flow centered on the selected asset.',
    steps: [
  ...
  'reports-exports': {
    ...
    intro: 'Use templates for repeatable outputs, runs for history and rerun, and delivery for light scheduling.',
    steps: [
  ...
  default: {
    ...
    intro: 'This page turns system docs and API metadata into task-oriented guidance, not just raw reference material.',
    steps: [
...
export default function HelpDocs() {
  ...
  const focusObject = searchParams.get('rule') || searchParams.get('asset') || searchParams.get('incident') || searchParams.get('q');
    ...
    <div>
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div className="summary-grid" style={{ marginTop: 16 }}>
          <div className="summary-card"><div className="summary-label">System Version</div><div className="summary-value">{statusData?.version || '—'}</div><div className="summary-meta">{hostData?.hostname || 'Unknown host'}</div></div>
          <div className="summary-card"><div className="summary-label">Context Scope</div><div className="summary-value">{focusObject || 'Workspace-level'}</div><div className="summary-meta">{focusObject ? 'Object-aware help is using the selected scope from the URL.' : 'Open “Help For View” from another workspace to carry its scope here.'}</div></div>
          <div className="summary-card"><div className="summary-label">Inbox Items</div><div className="summary-value">{inboxItems.filter((item) => !item.acknowledged).length}</div><div className="summary-meta">Durable operator work shared across the app shell and this support center.</div></div>
          <div className="summary-card"><div className="summary-label">Endpoints</div><div className="summary-value">{epArr.length}</div><div className="summary-meta">Live API metadata remains available below for technical support.</div></div>
        </div>
      </div>

      <div className="card-grid">
        <div className="card">
          <div className="card-title" style={{ marginBottom: 12 }}>Suggested Workflow</div>
        ...
        <div className="card">
          <div className="card-title" style={{ marginBottom: 12 }}>Operator Inbox</div>
          {inboxItems.length === 0 ? <div className="empty">No operator inbox items are active.</div> : (
            <div style={{ display: 'grid', gap: 10 }}>
              {inboxItems.map((item) => (
                <div key={item.id} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 12, opacity: item.acknowledged ? 0.65 : 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start' }}>
                    ...
                    <span className={`badge ${item.severity === 'high' ? 'badge-err' : 'badge-warn'}`}>{item.severity}</span>
                  </div>
                  <div className="hint" style={{ marginTop: 8 }}>
                    {item.created_at ? `${formatRelativeTime(item.created_at)} • ${formatDateTime(item.created_at)}` : 'Recently created'}
                  </div>
                  {!item.acknowledged && (
                    <button
        ...
        <div className="card">
          <div className="card-title" style={{ marginBottom: 12 }}>Runbook Links</div>
          <div style={{ display: 'grid', gap: 8 }}>
            <button className={`filter-chip-button ${context === 'threat-detection' ? 'active' : ''}`} onClick={() => setSearchParams({ context: 'threat-detection', rule: searchParams.get('rule') || '' }, { replace: true })}>How to tune this rule</button>
            <button className={`filter-chip-button ${context === 'infrastructure' ? 'active' : ''}`} onClick={() => setSearchParams({ context: 'infrastructure', asset: searchParams.get('asset') || '' }, { replace: true })}>How to investigate this asset</button>
            <button className={`filter-chip-button ${context === 'reports-exports' ? 'active' : ''}`} onClick={() => setSearchParams({ context: 'reports-exports' }, { replace: true })}>How to publish this report</button>
          </div>
          <div className="detail-callout" style={{ marginTop: 16 }}>
            <strong>What does this mean?</strong>
            <div style={{ marginTop: 6 }}>Severity describes urgency, lifecycle describes change readiness, and acknowledged inbox items stay visible so the team has a durable record of follow-up work.</div>
          </div>
        </div>
      </div>
      ...
      <div className="card-grid">
        <div className="card">
          <div className="card-title" style={{ marginBottom: 12 }}>Manager Context</div>
        ...
        <div className="card">
          <div className="card-title" style={{ marginBottom: 12 }}>API / Schema Reference</div>
