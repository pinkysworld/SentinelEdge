...
export default function NotificationToast() {
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => localStorage.getItem('wardex_notif_sound') !== 'false');
  ...
  const push = useCallback((message, kind = 'info') => {
    ...
    const dismissMs = kind === 'warning' || kind === 'error' ? CRITICAL_DISMISS_MS : DEFAULT_DISMISS_MS;
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 4 }}>
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: 'var(--text-secondary)', opacity: 0.6 }}
        >
          {soundEnabled ? '🔔' : '🔕'}
        </button>
      </div>
