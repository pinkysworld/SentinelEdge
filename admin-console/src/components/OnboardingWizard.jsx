...
function ChecklistItem({ label, complete, helper, actionLabel, onAction, busy }) {
  return (
    <div className={`onboarding-checklist-item ${complete ? 'complete' : ''}`}>
      <div className="onboarding-checklist-mark" aria-hidden="true">{complete ? '✓' : '○'}</div>
...
export default function OnboardingWizard({ onComplete }) {
  ...
  const [tokenCheck, setTokenCheck] = useState({ busy: false, ok: false, message: 'Validate the API token before finishing setup.' });
  const [feedCheck, setFeedCheck] = useState({ busy: false, ok: false, message: 'Select at least one feed and validate connectivity.' });
  const [telemetryCheck, setTelemetryCheck] = useState({ busy: false, ok: false, message: 'Confirm that the backend is already producing telemetry.' });
  ...
  const checklist = useMemo(() => ([
    {
      ...
      helper: token ? 'Token entered and ready to validate.' : 'Paste the admin token shown in the terminal.',
    },
    {
      key: 'token-verified',
      label: 'Verify token',
      ...
      onAction: async () => {
        ...
        try {
          ...
          setTokenCheck({ busy: false, ok: false, message: 'Token verification failed. Check the token and try again.' });
        } finally {
    ...
    },
    {
      ...
      helper: role ? `Workspace optimized for ${role}.` : 'Choose the workspace that matches your daily responsibilities.',
    },
    {
      key: 'feeds-validated',
      ...
      onAction: async () => {
        ...
        try {
          ...
          const sourceCount = stats?.total_sources ?? stats?.active_sources ?? selectedFeeds.length;
          setFeedCheck({
            busy: false,
            ok: selectedFeeds.length > 0,
            message: selectedFeeds.length > 0
    ...
    },
    {
      ...
      onAction: async () => {
        setTelemetryCheck({ busy: true, ok: false, message: 'Checking current telemetry volume…' });
        try {
          ...
          setTelemetryCheck({
            busy: false,
            ok: rate > 0 || total > 0,
            message: rate > 0 || total > 0
              ? `Telemetry is flowing (${rate || total} observed).`
              : 'No telemetry observed yet. You can finish setup and return once agents are online.',
          });
        } catch {
          setTelemetryCheck({ busy: false, ok: false, message: 'Telemetry check failed. Confirm the backend is running and retry.' });
        }
      },
  ...
  const toggleFeed = (feed) => {
    setSelectedFeeds((prev) => prev.includes(feed) ? prev.filter((item) => item !== feed) : [...prev, feed]);
    setFeedCheck({ busy: false, ok: false, message: 'Validate feed connectivity after updating the feed selection.' });
  };
      ...
      <div className="onboarding-panel" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">
        <div className="onboarding-header">
          <div>
            ...
            <p>This checklist gets the console ready for real analyst and admin workflows without hiding the important steps.</p>
          </div>
          <div className="onboarding-progress">
            <span className="onboarding-progress-count">{completedCount}/{checklist.length}</span>
        ...
        <div className="onboarding-grid">
          <section className="onboarding-section">
            <div className="form-group">
              <label className="form-label" htmlFor="onboarding-token">API Token</label>
              <input
                id="onboarding-token"
                ...
                onChange={(event) => {
                  ...
                  setTokenCheck({ busy: false, ok: false, message: 'Validate the API token before finishing setup.' });
                }}
                placeholder="Paste API token…"
                className="form-input"
                autoComplete="current-password"
              />
              <div className="form-helper">The token is printed in the terminal when the Wardex backend starts.</div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="form-helper">Analysts see queue-first workflows. Admins get fleet and configuration controls.</div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="form-helper">Pick the feeds you want configured as part of the initial setup.</div>
            </div>
          </section>
        ...
        <div className="onboarding-actions">
          <button type="button" className="btn btn-sm" onClick={onComplete}>Skip for now</button>
          <div className="btn-group">
            <button type="button" className="btn btn-sm btn-primary" onClick={finish} disabled={!canFinish}>
              Finish Setup
            </button>
          </div>
        </div>
