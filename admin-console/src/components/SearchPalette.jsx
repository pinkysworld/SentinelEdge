...
const CATEGORIES = [
  { key: 'alerts', label: 'Alerts', icon: 'AL', search: api.alerts, path: '/monitor', kind: 'entity' },
  { key: 'incidents', label: 'Incidents', icon: 'IN', search: api.incidents, path: '/soc', kind: 'entity' },
  { key: 'agents', label: 'Agents', icon: 'AG', search: api.agents, path: '/fleet', kind: 'entity' },
  { key: 'rules', label: 'Detection Rules', icon: 'RL', search: api.detectionRules, path: '/detection', kind: 'entity' },
  { key: 'feeds', label: 'Feed Sources', icon: 'FD', search: api.feeds, path: '/infrastructure', kind: 'entity' },
];

const COMMANDS = [
  { title: 'Create Incident', subtitle: 'Open the SOC workbench with a create flow', icon: 'CMD', action: 'create-incident', path: '/soc?intent=create-incident', category: 'Command', kind: 'action' },
  { title: 'Open Quarantine', subtitle: 'Jump to active response and quarantine work', icon: 'CMD', action: 'open-quarantine', path: '/soc?focus=quarantine', category: 'Command', kind: 'action' },
  { title: 'Run Hunt', subtitle: 'Open threat detection and start a hunt', icon: 'CMD', action: 'run-hunt', path: '/detection?intent=run-hunt', category: 'Command', kind: 'action' },
  { title: 'Review Offline Agents', subtitle: 'Open fleet with the offline status view', icon: 'CMD', path: '/fleet?status=offline', category: 'Command', kind: 'action' },
];
...
export default function SearchPalette({ open, onClose, onNavigate }) {
  ...
  const doSearch = useCallback(async (q) => {
    ...
    const allResults = COMMANDS.filter((command) => `${command.title} ${command.subtitle}`.toLowerCase().includes(ql));
      ...
      CATEGORIES.map(async (cat) => {
        try {
          ...
          items.forEach(item => {
            ...
            if (text.includes(ql)) {
              const title = item.name || item.hostname || item.title || item.id || item.alert_id || item.message || 'Unknown';
              const subtitle = item.severity || item.status || item.protocol || item.hostname || item.category || '';
              allResults.push({
                category: cat.label,
                icon: cat.icon,
                title,
  ...
  return (
    <div className="search-palette-overlay" onClick={() => onClose?.(false)}>
      <div className="search-palette" onClick={e => e.stopPropagation()}>
          ...
          <div style={{ padding: '8px 12px' }}>
            ...
            {saved.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 13 }}>
                <button className="search-saved-link" type="button" onClick={() => setQuery(s)}>Saved: {s}</button>
          ...
          <div className="search-command-grid">
            {COMMANDS.map((command) => (
              <button
                ...
                onClick={() => { onNavigate?.(command); onClose?.(false); }}
              >
        ...
        {loading && <div style={{ padding: 12, textAlign: 'center', color: 'var(--text-secondary)' }} role="status" aria-label="Searching">Searching…</div>}
        ...
        {results.length > 0 && (
          <div className="search-palette-results" id="search-results" role="listbox">
            {results.map((r, i) => (
              ...
              >
                <span className="search-result-icon" aria-hidden="true">{r.icon}</span>
                <div style={{ flex: 1 }}>
                ...
                <span className={`search-result-kind search-result-kind-${r.kind}`}>{r.kind === 'action' ? 'Action' : 'Entity'}</span>
              </div>
            ))}
          </div>
        )}
