...
- Endpoint-aware admin rate limiting with separate read/write budgets to avoid self-induced 429s under normal polling
- Slower telemetry-history refresh cadence in the admin console to reduce unnecessary API pressure
...
- 656 automated tests covering all 58 modules
...
- **Phase 25 — Code review hardening and security improvements** (T204–T207): Replaced insecure XOR spool cipher with SHA-256 CTR mode for encrypted event buffering, wired AuditLog into HTTP request handling for comprehensive API audit trail, activated RBAC enforcement on sensitive API endpoints, and removed the reviewed dead-code suppressions from the audit/RBAC paths while leaving 3 justified platform-specific suppressions in `collector.rs` and `service.rs`.
- **Phase 26 — Security audit fixes** (T208–T211): Reworked server rate limiting with separate read/write/static buckets so authenticated admin status traffic can poll safely without tripping the write budget, reduced admin console polling pressure by slowing live-monitor refresh cadence and throttling telemetry-history pulls, hardened the default CORS origin to `http://localhost`, and synchronised changelog/runtime bookkeeping with the actual security fixes applied.
...
- OpenAPI 3.0 specification with 149 endpoint definitions and `GET /api/openapi.json` public endpoint
- Schema lifecycle policy with versioning strategy, compatibility rules, and migration process
- Disaster recovery plan with backup/restore procedures, RTO/RPO, and DR validation tests
- Service-level objectives with error-budget instrumentation and `GET /api/slo/status` endpoint
- Deployment model documentation: standalone, multi-tenant, edge relay, regional federation
- Threat model promoted to primary docs: adversary profiles, abuse cases, trust boundaries
- Production hardening checklist: 59-control scorecard derived from XDR AI handoff pack

## Phase 28 — Production hardening (implemented)

- Token TTL with configurable session expiry (`security.token_ttl_secs`, default 1h)
- Token rotation endpoint (`POST /api/auth/rotate`) with immediate invalidation
- Session info endpoint (`GET /api/session/info`) with uptime, TTL, and expiry status
- Configurable retention policies for audit records, alerts, and events
- Retention status and apply endpoints (`GET /api/retention/status`, `POST /api/retention/apply`)
- Audit chain verification endpoint (`GET /api/audit/verify`)
- Spool per-tenant partitioning with `tenant_id` isolation
- mTLS configuration for agent connections (`security.require_mtls_agents`)
- Remote syslog forwarding endpoint configuration
- Dockerfile with multi-stage build, non-root user, and health check
- Docker Compose reference with security hardening
- Systemd service unit (`deploy/wardex.service`) with full Linux hardening
- Launchd plist (`deploy/com.wardex.agent.plist`) for macOS service deployment
- 5 chaos/fault injection integration tests
- 6 new API integration tests + 8 new unit tests

## v0.29.1 — Code review hardening (implemented)

- CORS origin validation: rejects wildcard `"*"` and non-HTTP scheme values
- CSV formula injection prevention in exported CSV files
- Admin console Session Management panel with session info display and token rotation
- Admin console Audit & Retention panel with chain verification and retention policy controls
- Auto-loading session/retention data when navigating to Settings
- 38-point code review covering security, error handling, logic, API design, and performance

## Practical milestone summary

The repository has completed all 28 phases (160/160 backlog tasks + Phase 28 hardening), providing a comprehensive edge security runtime that spans detection, enforcement, fleet orchestration, formal verification, privacy, post-quantum cryptography, and a professional browser-based admin console with 11 sections including a dedicated Research Tracks browser for the 40-track scientific roadmap. **v0.29.1** hardens the codebase from a 38-point code review: CORS origin validation rejects wildcard and non-HTTP origins, CSV export prevents formula injection, and the admin console gains Session Management and Audit & Retention panels in Settings. **v0.29.0** adds production hardening: token TTL with session expiry, token rotation, configurable retention policies, audit chain verification, spool per-tenant partitioning, mTLS configuration, Dockerfile with multi-stage build, systemd/launchd service units, and 5 chaos/fault injection tests. **v0.28.0** adds the Research Tracks panel to the admin UI (40 tracks across 8 groups with expandable cards and status filters), improves the monitoring scope UX with clear Main Server vs per-Agent labeling and a responsive grid layout for scope toggles, fixes 11 broken inline onclick handlers (IIFE scope bug), eliminates an XSS vector in dynamically generated buttons, and defines 3 missing CSS variables (`--danger`, `--green`, `--teal`). **Phase 27** delivers the full operational contract: OpenAPI 3.0 specification covering 149 endpoints, schema lifecycle documentation, disaster recovery plan, SLO policy with live metrics endpoint, deployment model guides (standalone/multi-tenant/edge relay/federation), a promoted threat model with abuse cases and trust boundaries, and a 59-control production hardening checklist scoring 80% implemented. See `docs/ROADMAP_XDR_PROFESSIONAL.md` for the professional feature roadmap beyond Phase 27. **Phase 26** fixes the remaining operational issues found during security review by separating rate-limit budgets for high-frequency admin reads vs. writes, reducing admin console polling pressure, hardening the default CORS origin to `http://localhost`, and reconciling stale status/changelog claims. **Phase 25** hardens the codebase with a SHA-256 CTR mode spool cipher replacing the insecure XOR cipher, comprehensive API audit logging via `AuditLog::record()` wired into every HTTP request, RBAC enforcement on sensitive endpoints, and accurate documentation of the remaining platform-specific dead-code suppressions.
